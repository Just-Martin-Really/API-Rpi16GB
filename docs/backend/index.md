...
src/                        Zig source
  ...
  metrics.zig               Prometheus exposition for backend
  handlers/
    health.zig              GET /health
    metrics.zig             GET /metrics
    sensor.zig              GET + POST /api/v1/sensor-data
    actuator.zig            POST /api/v1/actuator-command + GET/POST drain
    sensor_request.zig      POST /api/v1/sensor-request + GET/POST drain
docker/
  docker-compose.yml
  ...
  nginx/
    templates/conf.d/default.conf.template   TLS termination, rate limiting, subnet whitelist
    modsec/{main,custom-rules,exclusions}.conf   ModSecurity / OWASP CRS rules
  keycloak/
    iot-realm.json          realm import: clients, roles, service accounts
    keycloak_secrets.md     why client secrets are hardcoded in lab
  grafana/provisioning/     datasources (Prometheus + Postgres) + 6 dashboards
  prometheus/prometheus.yml scrape targets (backend, lstm, controller, postgres_exporter, node_exporter)
  dashboard/                static SPA (HTML + JS + CSS), bind-mounted into nginx
  postgres/
    init.sql                schema, DB users, grants (runs on first DB init)
    migrate.sql             one-off migration for the live Pi (archive table + phase-6 users + CHECKs)
  mosquitto/
  ...
  controller/
    controller.py           MQTT → HTTPS API bridge, Keycloak client-credentials auth
    Dockerfile
  lstm/Dockerfile           runs lstm/control_loop.py against the model artefacts
  archiver/
...
lstm/                       LSTM training + control loop sources, tests, requirements
docs/
  backend/                  architecture, setup, api, observability, keycloak, controller, …
  frontend/                 dashboard SPA docs
...
keycloak_controller_secret    # client secret for the Keycloak client-credentials flow
ca_cert
```
The Controller now:
...
4. authenticates to the Zig backend through nginx via a Keycloak access token (client-credentials flow as `controller-client`, realm role `controller-ingest`)
5. sends sensor readings to /api/v1/sensor-data (one row per measurement)
...
sh ./set_passwords.sh
```
The setup_tls.sh script generates CA and service certificates for nginx and Mosquitto. The set_passwords.sh script sets PostgreSQL user passwords and generates the Mosquitto password file.
