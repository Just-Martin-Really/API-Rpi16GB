...
def test_in_band_both_off():
    fc = [19.5, 20.0, 20.8, 20.5]
    assert desired_state(fc, low=19.0, high=21.0, lookahead=4) == {"heater": "off", "cooler": "off"}


def test_below_band_heater_on():
    fc = [20.5, 18.5, 20.0, 19.5]
    assert desired_state(fc, low=19.0, high=21.0, lookahead=4) == {"heater": "on", "cooler": "off"}


def test_above_band_cooler_on():
    fc = [20.0, 20.5, 22.1, 20.5]
    assert desired_state(fc, low=19.0, high=21.0, lookahead=4) == {"heater": "off", "cooler": "on"}
...
def test_low_takes_precedence_over_high():
    ...
    fc = [18.0, 22.0]
    assert desired_state(fc, low=19.0, high=21.0, lookahead=2) == {"heater": "on", "cooler": "off"}


def test_lookahead_clips_window():
    # Only the first 2 entries are considered. The cold dip at index 5 is ignored.
    fc = [20.5, 21.0, 22.0, 20.0, 19.5, 17.0]
    assert desired_state(fc, low=19.0, high=21.0, lookahead=2) == {"heater": "off", "cooler": "off"}


def test_boundary_low_inclusive_in_band():
    # min == low is not below low, so we stay in band.
    fc = [19.0, 20.0, 21.0]
    assert desired_state(fc, low=19.0, high=21.0, lookahead=3) == {"heater": "off", "cooler": "off"}
...
def test_invalid_lookahead_raises():
    with pytest.raises(ValueError):
        desired_state([20.0], low=19.0, high=21.0, lookahead=0)


def test_invalid_band_raises():
    with pytest.raises(ValueError):
        desired_state([20.0], low=21.0, high=19.0, lookahead=1)


def test_empty_forecast_raises():
    with pytest.raises(ValueError):
        desired_state([], low=19.0, high=21.0, lookahead=1)
